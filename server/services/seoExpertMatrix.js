...
function detectThemeCategory(techNames, html = '') {
  ...
  if (techNames.some((t) => t.includes('bootstrap')) || haystack.includes('bootstrap')) {
    return 'static';
  }
...
const INFRASTRUCTURE_CHECKS = [
  {
    ...
    severity: 'High',
    fix: 'Duplicate content rokne ke liye <link rel="canonical"> inject karein — http/https aur www variants ek hi URL par point karenge.',
  },
  {
    id: 'dns-prefetch',
    ...
    severity: 'High',
    fix: 'Google Fonts, CDN scripts ke liye dns-prefetch aur preconnect tags lagayein — Zero-RTT network response milega.',
  },
  {
    id: 'blocking-scripts',
    ...
    severity: 'Critical',
    fix: 'External <script> tags mein defer/async add karein — Lighthouse TBT score 90+ touch karega.',
  },
  {
    ...
    severity: 'Medium',
    fix: 'Chrome Speculation Rules inject karein — internal links instant prerender/prefetch honge.',
  },
  {
    id: 'robots-sitemap',
    ...
    severity: 'Critical',
    fix: 'Crawler path ke liye robots.txt aur sitemap.xml auto-generate karein — Google indexing 10x fast.',
  },
];
